...
public class BinarySearch {
    ...
    public static void main(String[] args) {
        ...
        //0 <= ans < args.length &&  args[ans + 1] <= elem &&
        // (args[ans] > elem || ans==0)
    ...
    //Pred  forall i in 0..arr.length arr[i+1] < arr[i]
    //Post 0 <= R < arr.length &&  arr[R] <= elem &&
    // (arr[R-1] > elem || R==0)
    public static int iterative(int[] arr, int elem) {
        ...
        while (end - start > 1) {
            ...
            index = (start + end) / 2;
            //start' <= index' <= end' &&
            // forall i in 0..(arr.length-1) arr[i+1] < arr[i]
            if (arr[index] > elem) {
        ...
        //  forall i in 0..end arr[i] > elem
        return end;
    }

    //Pred forall i in 0..args.length  arr[i+1] < arr[i]
    //Post 0 <= R < arr.length &&  arr[R] <= elem && (arr[R-1] > elem || R==0)
    ...
    //Post start <= R < end &&
    //  forall i in R..end arr[i] <= elem &&
    //  forall i in (start+1)..R arr[i] > elem
    private static int rec(int[] arr, int start, int end, int elem) {
        if (end - start > 1) {
            //end-start > 1 && (-1 <= start < end <= arr.length)
            int index = (start + end) / 2;
            //0 <= start < index < arr.length && index'!=index''
